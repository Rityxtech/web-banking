                                ...
                                {paginatedTransactions.length > 0 ? (paginatedTransactions.map((t) => {
                                    ...
                                    return (
                                        <tr key={t.id} onClick={() => setSelectedTx(t)} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors group cursor-pointer">
                                            ...
                                            <td className="px-6 py-4"><div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${t.status === 'Success' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 border-emerald-100 dark:border-emerald-900' : t.status === 'Pending' ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 border-amber-100 dark:border-amber-900' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 border-red-100 dark:border-red-800'}`}><span className={`w-1.5 h-1.5 rounded-full ${t.status === 'Success' ? 'bg-emerald-500' : t.status === 'Pending' ? 'bg-amber-500' : 'bg-red-500'}`}></span>{t.status || 'Success'}</div></td>
                                    ...
                                    return (
                                        <React.Fragment key={t.id}>
                                            ...
                                            <div onClick={() => setSelectedTx(t)} className="p-3 flex items-center justify-between active:bg-slate-50 dark:active:bg-slate-700/30 transition-colors cursor-pointer">
                                                ...
                                                <div className="text-right"><p className={`text-sm font-bold ${isNegative ? 'text-slate-900 dark:text-white' : 'text-emerald-600 dark:text-emerald-400'}`}>{isNegative ? '-' : '+'}${Math.abs(t.amount).toLocaleString('en-US', { minimumFractionDigits: 2 })}</p><div className="flex items-center justify-end gap-1 mt-1"><span className={`w-1.5 h-1.5 rounded-full ${t.status === 'Success' ? 'bg-emerald-500' : t.status === 'Pending' ? 'bg-amber-500' : 'bg-red-500'}`}></span><span className="text-[9px] text-slate-400 dark:text-slate-500">{t.status || 'Success'}</span></div></div>
                                            </div>
                                        </React.Fragment>
                                    );
                                })}
